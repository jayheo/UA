...
class UA_plus(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('output_v'):
            self.mu_weight = tf.get_variable('weight', shape=[self.embed_size, 1])
            self.mu_bias   = tf.get_variable('bias', shape=[1])
        ...
        #Mu
        self.mu = tf.matmul(c_i, self.mu_weight) + self.mu_bias
        
        #Sigma
        model_sigma = tf.matmul(c_i, self.sigma_weight) + self.sigma_bias        
        model_sigma = tf.nn.softplus(model_sigma)

        distribution = tf.distributions.Normal(loc=self.mu, scale=model_sigma)
        dist = distribution.sample([1])
        self.final_logits = tf.squeeze(dist, 0)
        self.preds = tf.nn.sigmoid(self.final_logits)
    ...
    def run(self):
        ...
        for i_ in range(self.max_epoch):
            ...
            if (i_+1)%self.save_iter == 0:
                save_path = saver.save(self.sess, 'checkpoints_UA_plus/'+self.task+'_'+str(i_+1)+'_%.4f'%(eval_loss)+'_%.4f'%(eval_auc)+'.ckpt')
